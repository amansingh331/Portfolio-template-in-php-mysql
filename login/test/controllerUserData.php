...
if(isset($_POST['signup'])){
    ...
    if(count($errors) === 0){
        ...
        $status = "notverified";
        $insert_data = "INSERT INTO usertable (name, email, password, code, status, user_ip_address, user_browser)
                        values('$name', '$email', '$password', '$code', '$status', '$IP', '$user_browser')";
        $data_check = mysqli_query($db, $insert_data);
        if($data_check){
            $subject = "Email Verification Code";
            $message = "Your verification code is $code";
            $sender = "From: no-reply@amansingh.xyz";
